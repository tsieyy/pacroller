...
class MailSender:
    def send_text_plain(self, text: str, subject: str = f"pacroller on {hostname}") -> Optional[bool]:
        if_failed = False
        if SMTP_ENABLED:
            if not self._send_email(text, subject, SMTP_TO.split()):
                if_failed = True
        if TG_ENABLED:
            if not self._send_tg_message(text, subject, TG_RECIPIENT.split()):
                if_failed = True
    ...
    @staticmethod
    def _send_email(text: str, subject: str, mailto: List[str]) -> bool:
    ...
    @staticmethod
    def _send_tg_message(text: str, subject: str, mailto: List[str]) -> bool:
        for _ in range(NETWORK_RETRY):
            all_succeeded = True
            try:
